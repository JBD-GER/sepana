...
const NAV = [{ href: "/versicherung", label: "Dashboard" }]

function isActive(pathname: string, href: string) {
  if (href === "/versicherung") return pathname === href
...
export default function InsuranceHeader({ initialEmail }: { initialEmail?: string | null }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
        <Link
          href="/versicherung"
          className="inline-flex items-center rounded-2xl border border-slate-200/80 bg-white px-3 py-2 shadow-sm"
        ...
        <nav className="hidden items-center gap-2 md:flex">
          ...
          {initialEmail ? (
            <div className="rounded-xl border border-slate-200/80 bg-white px-3 py-2 text-xs text-slate-600 shadow-sm">
              {initialEmail}
            </div>
          ) : null}
          <LogoutButton
      ...
      >
        <div className="mx-auto w-full max-w-7xl space-y-2 px-4 py-3 sm:px-6">
          ...
          {initialEmail ? (
            <div className="rounded-2xl border border-slate-200 bg-white px-3 py-3 text-xs text-slate-600 shadow-sm">
              {initialEmail}
            </div>
          ) : null}
          <LogoutButton
