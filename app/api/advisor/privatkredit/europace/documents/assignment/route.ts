...
export async function POST(req: Request) {
  try {
    ...
    const assignmentId = String(body?.assignmentId ?? "").trim() || null
    ...
    if (!category) {
      return NextResponse.json({ ok: false, error: "category fehlt." }, { status: 400 })
    }
    ...
    const assignment = await assignEuropaceDocumentForCase(admin, {
      caseId,
      vorgangsnummer,
      antragsnummer,
    ...
    return NextResponse.json({
      ...
      antragsnummer,
      documentId,
      assignedPages: assignment.assignedPages,
      documents: synced.documents,
      pages: synced.pages,
      uploadTargets,
    })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Europace-Zuordnung konnte nicht gesetzt werden."
